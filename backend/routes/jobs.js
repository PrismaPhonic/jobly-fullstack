...
/** DELETE /[id]/delete  {state} => {message: state} */

router.delete("/:id/apply", ensureCorrectUser, async function (req, res, next) {
  try {
    await Job.deleteApplication(req.params.id, req.username);
    console.log('got to delete route')
    return res.json({ message: `deleted job by id ${req.params.id}` });
  }
